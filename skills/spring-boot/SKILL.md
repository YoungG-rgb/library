---
name: spring-boot
description: Spring Boot 3.x development - REST APIs, JPA, Security, Testing, and Cloud-native patterns. Use for building enterprise Java applications with Spring Boot. Триггеры RU - «добавь контроллер», «создай сервис», «новый эндпоинт», «настрой Spring», «Spring Boot».
metadata:
  version: "2.0.0"
  domain: backend
...
Если в проекте есть каталог `.claude/standards/` — эти файлы являются истиной в последней инстанции и ПЕРЕОПРЕДЕЛЯЮТ любые рекомендации этого скилла при конфликте. Читай их ПЕРЕД работой:
...
- **Lombok**: на сущностях — `@Getter @Setter @NoArgsConstructor @AllArgsConstructor @Accessors(chain = true) @FieldDefaults(level = PRIVATE)`. **Никогда `@Data` / `@Builder`**. Сервисы — `@RequiredArgsConstructor` + `private final`.
- **Раскладка**: `<feature>/{communications,domain,repositories,services,config}` — НЕ корневые `controller/service/repository/model`.
- **DI**: только конструкторная. Field `@Autowired` допустим **только** в prototype-scoped Task-бинах (см. `scheduler.md`).
- **Время**: `LocalDateTime` в JPA. Дневное окно / DST — через `ZonedDateTime`, никаких `+24h` арифметикой. Никаких `Instant` в сущностях.
- **PII в логах**: msisdn / PAN / email — только через маскер проекта. Никогда в исходном виде. Никогда в MDC.
- **CLAUDE.md** проекта — всегда главнее. Прочитай его перед структурными изменениями.

Если `.claude/standards/` отсутствует — используй generic-контент ниже как fallback.
