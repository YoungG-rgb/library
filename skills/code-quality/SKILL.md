---
name: code-quality
description: Comprehensive code review for Java - clean code principles, API contracts, null safety, exception handling, and performance. Use when user says "review code", "refactor", "check API", or before merging changes. Триггеры RU - «проверь код», «ревью», «отрефактори», «почини», «проверь API».
---

## ⚠️ Project Standards Override

Если в проекте есть `.claude/standards/` — эти файлы являются истиной в последней инстанции и переопределяют generic-рекомендации ниже при конфликте.

| Тема                       | Файл                                          |
|----------------------------|-----------------------------------------------|
| Структура пакетов          | `.claude/standards/package-structure.md`      |
...
- **Время**: `LocalDateTime` в сущностях/репозиториях, `ZonedDateTime` для дневного окна и DST. Никаких `Instant` в JPA и никаких `+24h` арифметикой.
- **DI**: только конструкторная (`@RequiredArgsConstructor` + `private final`). Field `@Autowired` — флаг ревьюера, кроме prototype-Task.
- **Lombok**: не `@Data`, не `@Builder` на сущностях. Должны быть `@Getter @Setter @NoArgsConstructor @AllArgsConstructor @Accessors(chain = true) @FieldDefaults(level = PRIVATE)`.
- **Self-invocation** между `@Transactional`-методами одного бина — критичный флаг.
- **Идемпотентность**: каждый side-effect должен иметь UNIQUE-индекс / стабильный idempotency-key.
- **Чек-листы перед PR**: цитируй пункты из соответствующих `.claude/standards/*.md` в выводе ревью.
