...
## Подключение Lombok (обязательно, делай это при создании проекта — не позже)

Этот стандарт **требует** Lombok (`@RequiredArgsConstructor`, `@Slf4j` — ниже). Сам по себе
он в проект не попадает — на шаге «выбор инструментов»/bootstrap (до первого сервиса)
добавь зависимость явно, до того как написан хоть один `@Service`:
...
Версией управляет `spring-boot-starter-parent` (dependencyManagement) — отдельно версию
не фиксируй. Аннотационный процессор Maven подхватывает с classpath автоматически,
доп. `annotationProcessorPaths` нужен только при совместном использовании с MapStruct
(см. `web.md` референс). Gradle: `compileOnly 'org.projectlombok:lombok'` +
`annotationProcessor 'org.projectlombok:lombok'`.

**Почему это отдельный обязательный шаг, а не сноска:** стандарт годами может требовать
`@RequiredArgsConstructor`, но если зависимость не добавлена в первый час жизни проекта,
первый же сервис пишется с ручным конструктором — и все следующие копируют этот локальный
паттерн, а не сверяются с файлом стандарта. Разрыв «стандарт есть, зависимости нет»
не ловится сборкой и обнаруживается только на ревью, если вообще обнаруживается.
Чек-лист bootstrap-а проекта (см. также корневой `CLAUDE.md`) должен явно включать
пункт «добавлен Lombok» — до, а не после написания сервисов.
