...
- **Нет `checkstyle.xml` в корне репозитория — стиль не проверяется, и это штатная ситуация.**
  Не заводи конфиг только ради того, чтобы стандарт считался выполненным. Не подключай плагин
  к отсутствующему файлу. Не считай его отсутствие нарушением на код-ревью и не заводи задачу
  «добавить checkstyle» по своей инициативе.
  - **Почему:** плагин с `configLocation` на несуществующий файл валит сборку на каждом модуле.
    А конфиг, добавленный «на всякий случай» к уже написанному коду, сразу даёт поток нарушений —
    и команда начинает глушить правила вместо того, чтобы их соблюдать.
- Всё, что ниже, — инструкция **как подключить** Checkstyle, если проект решил его подключать.
  Это не требование его подключить.
- Решение «в этом проекте Checkstyle не используем» фиксируется один раз в логе решений проекта,
  а не обсуждается заново на каждом ревью.
...
Применяется, **только если проект подключает Checkstyle**. Нет `checkstyle.xml` в корне —
чек-лист пропускается целиком.
