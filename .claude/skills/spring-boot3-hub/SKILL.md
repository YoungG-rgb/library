...
**Если `.claude/standards/` есть, а корневого `CLAUDE.md` нет — заведи его первым шагом,
до первой строчки инфраструктурного кода** (`pom.xml`/`build.gradle`, security-конфиг,
первый `@Service`). Минимум внутри: ссылка на `.claude/standards/` как источник истины,
bootstrap-чек-лист обязательных зависимостей, которые стандарты подразумевают, но сами
не тянут (типичный пример — Lombok для `@RequiredArgsConstructor`/`@Slf4j` из
`service-transactional.md`: стандарт можно годами не нарушать явно, если зависимость
просто никогда не добавили), и явная пометка, если пакетная схема проекта отличается от
generic `package-structure.md` (например DDD-вариант `api/application/domain/infrastructure`
вместо `communications/domain/repositories/services`). Без этого шага стандарты
проверяются только реактивно (когда триггернулся конкретный скилл по ключевым словам),
а не как единый чек-лист перед первым коммитом — и типовой Spring Boot паттерн из общих
знаний тихо побеждает то, что уже написано в `.claude/standards/`.
